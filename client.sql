/*
 ...
 Source Server         : 永红源test
 Source Server Type    : MySQL
 Source Server Version : 50728
 Source Host           : 10.110.16.3:13306
...
CREATE TABLE `yhy_certificate` (
  ...
  `employee_id` char(25) NOT NULL COMMENT '员工id',
  `certificate_name` varchar(50) NOT NULL COMMENT '证书名称',
  `certificate_num` varchar(100) NOT NULL COMMENT '证件编号',
  `cert_begin_time` int(11) NOT NULL COMMENT '证书开始时间',
  `cert_end_time` int(11) NOT NULL COMMENT '证书结束时间',
  `cert_resource_id` char(25) NOT NULL COMMENT '证书图片',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`certificate_id`),
  KEY `idx_employee_is` (`employee_id`) USING BTREE COMMENT '员工id'
...
CREATE TABLE `yhy_emergency` (
  ...
  `employee_id` char(25) NOT NULL COMMENT '员工id',
  `emergency_name` varchar(20) NOT NULL COMMENT '紧急联系人',
  `relationship` varchar(20) NOT NULL COMMENT '关系',
  `emergency_phone` varchar(13) NOT NULL COMMENT '紧急联系人电话',
  `update_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`emergency_id`),
  KEY `idx_employee_id` (`employee_id`) USING BTREE COMMENT '员工id'
...
CREATE TABLE `yhy_employee` (
  `employee_id` char(25) NOT NULL COMMENT '用户自增id',
  `sex` int(11) NOT NULL COMMENT '性别',
  `user_type_tag_id` int(11) NOT NULL COMMENT '用户类型',
  `full_name` varchar(20) NOT NULL DEFAULT '' COMMENT '全名',
  `nick_name` varchar(20) NOT NULL DEFAULT '' COMMENT '昵称',
  `user_name` varchar(20) NOT NULL DEFAULT '' COMMENT '用户名',
  `mobile` varchar(13) NOT NULL COMMENT '手机号',
  `status` enum('Y','N') NOT NULL DEFAULT 'Y' COMMENT '状态',
  `autolock` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '是否锁定',
  `editor` char(25) NOT NULL COMMENT '修改者',
  `creator` char(25) NOT NULL COMMENT '创建者',
  `verify` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '是否验证',
  `app_id` varchar(25) NOT NULL COMMENT '应用id',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`employee_id`) USING BTREE,
  UNIQUE KEY `eu_mobile` (`mobile`) USING BTREE,
  UNIQUE KEY `eu_username` (`user_name`) USING BTREE
...
CREATE TABLE `yhy_employee_ext` (
  ...
  `project_id` char(25) NOT NULL COMMENT '项目id',
  ...
  KEY `index_employee_id` (`employee_id`) USING BTREE COMMENT '员工id'
) ENGINE=InnoDB AUTO_INCREMENT=70 DEFAULT CHARSET=utf8 COMMENT='附加信息表';
...
CREATE TABLE `yhy_member` (
  ...
  `employee_id` char(25) NOT NULL DEFAULT '0' COMMENT '用户id',
  `type` enum('0','1','2','3','4') NOT NULL DEFAULT '1' COMMENT '登陆类型，1为oa登陆 2位账号密码登陆,4 自动登录',
  ...
  `oa` varchar(50) NOT NULL DEFAULT '' COMMENT 'oa账号',
  `is_lock` enum('0','1') NOT NULL DEFAULT '0' COMMENT '是否锁定 1为锁定',
  `last_login_project_id` char(25) DEFAULT '0',
  `last_login_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '上次登陆时间',
  `last_login_ip` varchar(15) NOT NULL COMMENT '上次登陆ip',
  `app_id` varchar(25) NOT NULL COMMENT '应用id',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`member_id`),
  KEY `user_id` (`employee_id`) USING BTREE
...
CREATE TABLE `yhy_tenement` (
  `tenement_id` char(25) NOT NULL COMMENT '自增id',
  `user_id` int(11) NOT NULL COMMENT '用户id',
  `license_tag_id` int(11) NOT NULL COMMENT '证件类别',
  `license_num` varchar(20) NOT NULL COMMENT '证件号码',
  ...
  `pet_type_tag_id` varchar(20) NOT NULL COMMENT '宠物类型',
  `face_resource_id` char(25) NOT NULL COMMENT '照片id',
  `pet_remark` varchar(300) NOT NULL COMMENT '宠物备注',
  `pet_num` int(5) NOT NULL COMMENT '宠物数量',
  `account_name` varchar(50) NOT NULL DEFAULT '' COMMENT '账户名称',
  `tax_number` varchar(25) NOT NULL COMMENT '纳税人识别号',
  `native_place` varchar(50) NOT NULL COMMENT '籍贯',
  `bank` varchar(50) NOT NULL COMMENT '银行',
  `back_account` varchar(50) NOT NULL COMMENT '银行账号',
  `back_name` varchar(50) NOT NULL COMMENT '开户行',
  `tax_address` varchar(100) NOT NULL COMMENT '纳税人地址',
  `corporation` varchar(50) NOT NULL COMMENT '工作单位',
  `creator` char(25) NOT NULL COMMENT '创建者',
  `editor` char(25) NOT NULL COMMENT '修改者',
...
CREATE TABLE `yhy_tenement_car` (
  ...
  `tenement_id` char(25) NOT NULL COMMENT '用户id',
  `space_name` varchar(50) NOT NULL COMMENT '车位编号',
  `plate` varchar(50) NOT NULL COMMENT '车牌号',
  `car_type_tag_id` int(11) NOT NULL COMMENT '车辆类型',
  `car_type` int(25) NOT NULL COMMENT '车辆型号',
  `car_type_name` varchar(25) NOT NULL COMMENT '车辆型号名称',
  `car_model` int(25) NOT NULL COMMENT '车型',
  `car_brand` int(25) NOT NULL COMMENT '车辆品牌',
  `car_brand_name` varchar(25) NOT NULL COMMENT '车辆品牌名称',
  `rule` varchar(100) NOT NULL COMMENT '月卡规则',
  `car_resource_id` char(25) NOT NULL COMMENT '资源id',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`driver_id`),
  UNIQUE KEY `UK_plate_space` (`tenement_id`,`space_name`,`plate`) USING BTREE COMMENT '唯一索引'
...
CREATE TABLE `yhy_tenement_house` (
  ...
  `tenement_id` char(25) NOT NULL COMMENT '业主id',
  `cell_id` char(25) NOT NULL DEFAULT '0' COMMENT '室id',
  `house_id` char(25) NOT NULL COMMENT '房产id',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`t_house_id`),
  UNIQUE KEY `UK_tene_house` (`tenement_id`,`house_id`,`cell_id`) USING BTREE COMMENT '唯一索引'
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8;
...
CREATE TABLE `yhy_tenement_label` (
  ...
  `tenement_id` char(25) NOT NULL COMMENT '住户id',
...
CREATE TABLE `yhy_visitor` (
  `visit_id` char(25) NOT NULL COMMENT '访客id',
  `user_id` int(11) NOT NULL COMMENT '用户id',
  `sfz_number` varchar(25) NOT NULL COMMENT '身份证号码',
  `real_name` varchar(20) NOT NULL COMMENT '真实姓名',
  `sex` int(11) NOT NULL COMMENT '性别',
  `project_id` char(25) NOT NULL COMMENT '项目id',
  `mobile` varchar(13) NOT NULL COMMENT '手机号',
  `appoint_time` int(11) NOT NULL COMMENT '预约时间',
  `is_man_face` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '是否人脸，默认''N''',
  `appoint_status_tag_id` int(11) NOT NULL COMMENT '预约状态',
  `face_resource_id` char(25) NOT NULL COMMENT '人脸id',
  `in_time` int(11) NOT NULL DEFAULT '0' COMMENT '进入时间',
  `out_time` int(11) NOT NULL DEFAULT '0' COMMENT '出去时间',
  `authorizer` varchar(20) NOT NULL COMMENT '授权人',
  `creator` char(25) NOT NULL COMMENT '创建者',
  `editor` char(25) NOT NULL COMMENT '修改者',
...
CREATE TABLE `yhy_visitor_car` (
  ...
  `visit_id` char(25) NOT NULL COMMENT '用户id',
  `plate` varchar(50) NOT NULL COMMENT '车牌号',
  `car_type_name` varchar(25) NOT NULL COMMENT '车辆型号名称',
  `car_type` int(11) NOT NULL COMMENT '车辆型号id',
  `car_brand` int(11) NOT NULL COMMENT '车辆品牌id',
  `car_brand_name` varchar(25) NOT NULL COMMENT '品牌名称',
  `car_model` varchar(25) NOT NULL COMMENT '车型',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`visit_car_id`) USING BTREE,
  UNIQUE KEY `UK_visitor_plate` (`visit_id`,`plate`) USING BTREE COMMENT '唯一id'
...
CREATE TABLE `yhy_visitor_follow` (
  ...
  `visit_id` char(25) NOT NULL COMMENT '访客id',
  `follow_name` varchar(20) NOT NULL COMMENT '到访人员姓名',
  `follow_mobile` varchar(13) NOT NULL COMMENT '到访人员手机号',
  `create_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
